...
namespace edu_simple
{
    ...
    public interface IExcellentInfo
    {
        void PrintExcellenceInfo();
    }

    public delegate void StudentMethodsDelegate();

    public class Student : IStudentInfo, IExcellentInfo
    {
        public string Id;
        ...
        public void PrintMarks()
        {
            ...
            foreach (var kv in Marks)
            {
                ...
                if (++k < Marks.Count) Console.Write(", ");
            }
            Console.WriteLine();
        }

        public void PrintExcellenceInfo()
        {
            Console.WriteLine($"Отличник: {IsExcellent()}");
        }
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            while (true)
            {
                ...
                switch (cmd)
                {
                    ...
                    case "10": TestStudentInterfaces(); break;
        ...
        static void Menu()
        {
            ...
            Console.WriteLine("9) Запрос: полный список отличников (в файл)");
            Console.WriteLine("10) Тест интерфейсов и делегата для студента");
                ...
                foreach (var course in inst.Courses)
                {
                    foreach (var group in course.Groups)
                    {
                        foreach (var st in group.Students)
                        {
                            ...
                            {
                                lines.Add(
                                    $"{st.FullName} (ID:{st.Id}) — " +
                                    $"Институт: {inst.Name}, Курс: {course.Number}, Группа: {group.Name}"
                                );
                            }
                        }
                    }
        ...
        static void TestStudentInterfaces()
        {
            var inst = PickInstitute(); if (inst == null) return;
            var course = PickCourse(inst); if (course == null) return;
            var group = PickGroup(course); if (group == null) return;
            ...
            IStudentInfo studentInfo = st;
            IExcellentInfo excellentInfo = st;

            StudentMethodsDelegate methods = studentInfo.PrintShortInfo;
            methods += studentInfo.PrintMarks;
            methods += excellentInfo.PrintExcellenceInfo;

            Console.WriteLine("Проверка методов студента через многоадресный делегат:\n");
            methods();
            Console.WriteLine();
        }
